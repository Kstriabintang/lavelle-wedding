<script setup>
// TEMPLATE ADAT — renderer undangan adat Nusantara BERBASIS DATA (versi portal).
// Suku (minang/jawa/sunda/bugis) dipilih via data.adat.suku → membawa palet, motif,
// salam, prosesi, karikatur, & musik khas suku (dari src/data/adat.js). Data pasangan
// (nama/foto/tanggal/kisah/hadiah) diambil dari data portal, menimpa contoh suku.
...
import { imgSrc } from '../../components/royale/imgSrc.js'
...
const suku = computed(() => (r.value.adat && r.value.adat.suku) || 'minang')
const base = computed(() => adat[suku.value] || adat.minang)
const nz = (v, fb) => (v !== undefined && v !== null && String(v).trim() ? v : fb)

// cfg = gabungan: konten budaya khas suku + data pasangan dari portal (bila diisi).
const cfg = computed(() => {
  const b = base.value
  const h = r.value.hero
  const story = (r.value.story || []).filter((s) => s.title || s.desc)
  return {
    slug: b.slug, suku: b.suku, tagline: b.tagline, palette: b.palette, motif: b.motif,
    salam: b.salam,
    quote: nz(r.value.quote && r.value.quote.text, b.quote),
    quoteRef: nz(r.value.quote && r.value.quote.ref, b.quoteRef),
    bride: nz(h.bride, b.bride), groom: nz(h.groom, b.groom),
    brideFull: nz(r.value.bride && r.value.bride.name, b.brideFull),
    groomFull: nz(r.value.groom && r.value.groom.name, b.groomFull),
    brideParents: nz(r.value.bride && r.value.bride.parents, b.brideParents),
    groomParents: nz(r.value.groom && r.value.groom.parents, b.groomParents),
    caricatureBride: b.caricatureBride, caricatureGroom: b.caricatureGroom,
    date: nz(h.date, b.date), dateText: nz(h.dateText, b.dateText),
    akadTime: nz(r.value.events && r.value.events[0] && r.value.events[0].time, b.akadTime),
    resepsiTime: nz(r.value.events && r.value.events[1] && r.value.events[1].time, b.resepsiTime),
    venue: nz((r.value.events && ((r.value.events[1] && r.value.events[1].place) || (r.value.events[0] && r.value.events[0].place))), b.venue),
    mapsQuery: nz(r.value.mapsQuery, b.mapsQuery),
    prosesi: b.prosesi,
    loveStory: story.length ? story.map((s) => ({ year: s.year, title: s.title, desc: s.desc })) : b.loveStory,
    liveStream: b.liveStream,
    coverPhoto: b.coverPhoto,
    turutMengundang: (r.value.family && r.value.family.alsoInviting && r.value.family.alsoInviting.length) ? r.value.family.alsoInviting : b.turutMengundang,
    accent: b.accent,
  }
})
const p = computed(() => cfg.value.palette)
const initials = computed(() => `${cfg.value.bride[0]} & ${cfg.value.groom[0]}`)
const pageTitle = computed(() => `Undangan Pernikahan ${cfg.value.bride} & ${cfg.value.groom}`)

// Foto: pakai URL portal bila ada; jika kosong pakai contoh /img/mentahan (via imgSrc).
const coverUrl = computed(() => r.value.hero.photo || imgSrc(base.value.coverPhoto))
const gallery = computed(() => {
  const g = (r.value.gallery || []).map((x) => x.src).filter(Boolean)
  return g.length ? g : adatGallery.map((n) => imgSrc(n))
})
const photobreakUrl = computed(() => gallery.value[3] || gallery.value[0] || coverUrl.value)
...
// Cover
const opened = ref(!!props.preview)
function openInvite() {
  opened.value = true
...
function sendRsvp() {
  ...
  // Simpan ucapan ke dinding + localStorage (lokal; backend Supabase menyusul).
...
const gifts = computed(() => {
  const gp = r.value.gifts || []
  if (gp.length) return gp.map((g) => ({ bank: g.label, no: g.no || g.raw, raw: g.raw || g.no, an: g.an }))
  return [
...
const musicSrc = computed(() => r.value.music.src || MUSIC[cfg.value.slug] || '/demo/sinema/audio/risk-it-all-piano.mp3')
...
<template>
  <div class="adat" :style="paletteVars" :data-motif="cfg.motif">
    ...
    <div v-if="coverUrl" class="a-deskbg" :style="{ backgroundImage: `url(${coverUrl})` }"></div>
    ...
    <transition name="cover">
      <section v-if="!opened" class="a-cover" :class="{ 'a-cover--photo': coverUrl }">
        <template v-if="coverUrl">
          <div class="a-cover__photo" :style="{ backgroundImage: `url(${coverUrl})` }"></div>
    ...
    <main class="a-main">
      ...
      <section class="a-sec">
        <div class="a-wrap a-center">
          ...
          <figure v-if="coverUrl" class="a-couple-hero a-reveal">
            <img :src="coverUrl" :alt="`${cfg.bride} & ${cfg.groom}`" loading="lazy">
      ...
      <section class="a-photobreak" :style="{ backgroundImage: `url(${photobreakUrl})` }">
          ...
          <div class="a-slider-wrap a-reveal">
            ...
            <div class="a-slider" ref="sliderEl" @scroll.passive="onSliderScroll">
              <button v-for="(g, i) in gallery" :key="g" class="a-slide" type="button" @click="openLb(i)" aria-label="Perbesar foto">
                <img :src="g" :alt="`Momen ${i + 1}`" loading="lazy">
      ...
      <section class="a-sec a-sec--paper">
        <div class="a-wrap a-center">
          ...
          <form class="a-form a-reveal" @submit.prevent="sendRsvp">
            ...
            <button class="a-btn a-btn--gold" type="submit">
              <i class="fa-solid" :class="rsvpSent ? 'fa-circle-check' : 'fa-paper-plane'"></i>
              {{ rsvpSent ? 'Terima kasih! Konfirmasi terkirim' : 'Kirim Ucapan &amp; Konfirmasi' }}
            </button>
            <small>Ucapan &amp; doa Anda akan tampil di dinding ucapan di bawah.</small>
          </form>
    ...
    <transition name="lb">
      <div v-if="lb >= 0" class="a-lightbox" @click.self="closeLb">
        <img :src="gallery[lb]" :alt="`Momen ${lb + 1}`">
